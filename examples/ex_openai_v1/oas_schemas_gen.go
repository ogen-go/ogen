...
type CreateAnswerRequest struct {
	...
	// A text snippet containing the contextual information used to generate the answers for the `examples`
	// you provide.
	...
	// The ID of an uploaded file that contains documents to search over. See [upload file] for how to
	// upload a file of the desired format and purpose.
	//
	// You should specify either `documents` or a `file`, but not both.
	//
	// [upload file]: /docs/api-reference/files/upload
	File OptNilString `json:"file"`
	// ID of the model to use for [Search]. You can select one of `ada`, `babbage`, `curie`, or `davinci`.
	//
	// [Search]: /docs/api-reference/searches/create
	SearchModel OptNilString `json:"search_model"`
	// The maximum number of documents to be ranked by [Search] when using `file`. Setting it to a higher
	...
	// generated.
	//
	// [tokenizer tool]: /tokenizer?view=bpe
	...
	// If an object name is in the list, we provide the full information of the object; otherwise, we only
	// provide the object ID. Currently we support `completion` and `file` objects for expansion.
...
type CreateChatCompletionRequest struct {
	...
	// The maximum number of tokens allowed for the generated answer. By default, the number of tokens the
	// model can return will be (4096 - prompt tokens).
...
type CreateClassificationRequest struct {
	...
	// [Model overview]: /docs/models/overview
	Model string `json:"model"`
	...
	// The ID of the uploaded file that contains training examples. See [upload file] for how to upload a
	...
	// collected from the examples you provide. All the label strings will be normalized to be capitalized.
	...
	// [Help center]: https://help.openai.com
	Logprobs OptNilInt `json:"logprobs"`
	// The maximum number of examples to be ranked by [Search] when using `file`. Setting it to a higher
	// value leads to improved accuracy but with increased latency and cost.
	//
	// [Search]: /docs/api-reference/searches/create
...
type CreateFileRequestMultipart struct {
	...
	//
	// [JSON Lines]: https://jsonlines.readthedocs.io/en/latest/
	...
	//
	// [Fine-tuning]: /docs/api-reference/fine-tunes
...
type CreateFineTuneRequest struct {
	...
	//
	// Your dataset must be formatted as a JSONL file, where each training example is a JSON object with
	// the keys "prompt" and "completion". Additionally, you must upload your file with the purpose
	// `fine-tune`.
	//
	// See the [fine-tuning guide] for more details.
	//
	// [upload file]: /docs/api-reference/files/upload
	// [fine-tuning guide]: /docs/guides/fine-tuning/creating-training-data
	...
	//
	// Your dataset must be formatted as a JSONL file, where each validation example is a JSON object with
	// the keys "prompt" and "completion". Additionally, you must upload your file with the purpose
	// `fine-tune`.
	//
	// See the [fine-tuning guide] for more details.
	...
	// or a fine-tuned model created after 2022-04-21. To learn more about these models, see the [Models]
	// documentation.
	//
	...
	// the training set, capped at 256 - in general, we've found that larger batch sizes tend to work
	...
	// The weight to use for loss on the prompt tokens. This controls how much the model tries to learn to
	// generate the prompt (as compared to the completion which always has a weight of 1.0), and can add a
	...
	//
	// In order to compute classification metrics, you must provide a `validation_file`. Additionally, you
	// must specify `classification_n_classes` for multiclass classification or
...
type CreateTranscriptionRequestMultipart struct {
	...
	//
	// [prompt]: /docs/guides/speech-to-text/prompting
