...
type Board struct {
	...
	// Массив иконок, которые доступны на этой доске, если они
	// включены.
...
type Captcha struct {
	...
	// Тип текста, изображённого на картинке капчи. Возможные
	// варианты:
	//
...
//   - 2chcaptcha: Два.ч капча 2chcaptcha_id - идентификатор Два.ч капчи.
//     2chcaptcha_value - строка, которую пользователь увидел на
//     картинке.
//   - appid: app_response_id и app_response app_response_id - результат запроса к
//     этому методу с публичным ключём приложения:
//     /api/captcha/app/id/{public_key} app_response - sha256(app_response_id + '|' + private_key)
