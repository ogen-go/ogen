...
func TestTypeExtension_Params(t *testing.T) {
	...
	t.Run("Defaults", func(t *testing.T) {
		expected := api.OptionalParams{
			OgenString: api.NewOptStringOgen(testtypes.StringOgen{Value: "1.1"}),
			OgenNumber: api.NewOptNumberOgen(testtypes.NumberOgen{Value: 1.2}),
			JsonString: api.NewOptStringJSON(testtypes.StringJSON{Value: "1.3"}),
			JsonNumber: api.NewOptNumberJSON(testtypes.NumberJSON{Value: 1.4}),
			TextString: api.NewOptText(testtypes.Text{Value: "1.5"}),
			TextNumber: api.NewOptText(testtypes.Text{Value: "1.6"}),
			String:     api.NewOptString(testtypes.String("1.7")),
			Number:     api.NewOptNumber(testtypes.Number(1.8)),
			Alias:      api.NewOptAlias(api.Alias{Value: "1.9"}),
		}
